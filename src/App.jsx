...
const SectionHeader = ({ title, subtitle, align = "center", light = false }) => (
  <div className={`mb-12 md:mb-16 ${align === "center" ? "text-center" : "text-left"}`}>
    <h3 className={`text-xs md:text-sm font-bold tracking-widest uppercase mb-3 ${light ? "text-stone-400" : "text-stone-500"}`}>
      {subtitle}
    </h3>
    <h2 className={`text-3xl md:text-4xl font-serif leading-tight ${light ? "text-white" : "text-stone-800"}`}>
      {title}
    </h2>
    <div className={`h-1 w-20 md:w-24 bg-stone-400 mt-4 md:mt-6 ${align === "center" ? "mx-auto" : ""}`}></div>
  </div>
);

const Card = ({ children, className = "" }) => (
  <div className={`p-6 md:p-8 rounded-sm border border-stone-200/60 shadow-sm hover:shadow-md transition-all duration-300 ${className}`}>
    {children}
  </div>
);
...
const Navbar = () => {
  ...
  return (
    // FONDO BEIGE SUAVE
    <nav className="bg-[#F5F2EB] border-b border-stone-200 py-4 sticky top-0 z-50 shadow-sm">
      <div className="container mx-auto px-6 flex justify-between items-center">
        <div className="flex items-center gap-2">
          {/* ICONO EN GRIS */}
          <div className="bg-stone-600 p-2 rounded-md text-white shadow-sm">
            <Activity size={20} />
          </div>
          <div>
            <span className="text-lg md:text-xl font-serif text-stone-800 block leading-none">Dra. Altamirano</span>
        ...
        <div className="hidden md:flex space-x-8 text-sm font-medium text-stone-600">
          {['Inicio', 'Perfil', 'Servicios', 'Trayectoria', 'Contacto'].map((item) => (
            <a key={item} href={`#${item.toLowerCase()}`} className="hover:text-stone-900 transition">{item}</a>
          ))}
        </div>
        
        <a 
          href={emailLink}
          className="hidden md:block bg-stone-700 text-white px-6 py-2.5 text-sm font-medium hover:bg-stone-600 transition rounded-sm shadow-sm"
        >
          Pedir Turno
        </a>
      ...
      {/* Mobile Menu */}
      {isOpen && (
        <div className="md:hidden bg-[#F5F2EB] border-t border-stone-200 absolute w-full left-0 top-full shadow-xl z-50 animate-fade-in-down">
          <div className="flex flex-col p-6 space-y-4">
            {['Inicio', 'Perfil', 'Servicios', 'Trayectoria', 'Contacto'].map((item) => (
              <a 
                ...
                className="text-stone-600 font-medium text-lg border-b border-stone-200 pb-2"
              >
                {item}
              </a>
            ))}
            <a 
              href={emailLink} 
              className="bg-stone-700 text-white text-center py-3 rounded font-medium mt-4"
            >
               Pedir Turno (Email)
            </a>
          </div>
...
const Hero = () => {
  ...
  return (
    ...
    <section id="inicio" className="relative bg-[#F5F2EB] text-stone-800 min-h-screen flex items-center justify-center overflow-hidden pt-12 md:pt-0">
      
      {/* Fondo Abstracto: Formas en beige un poco más oscuro para textura */}
      ...
      <div className="container mx-auto px-6 relative z-10 grid md:grid-cols-2 gap-12 items-center mt-0 md:-mt-32 pb-20 md:pb-0">
        ...
        <div className="space-y-6 md:space-y-8 animate-fade-in-up text-center md:text-left">
          <div className="inline-flex items-center gap-2 px-3 py-1 border border-stone-400/30 rounded-full bg-white/50 text-stone-600 text-xs font-bold tracking-widest uppercase">
            <Star size={12} /> Excelencia Médica
          </div>
          
          <h1 className="text-4xl md:text-7xl font-serif leading-tight text-stone-900">
            Cuidando el futuro <br/>
            <span className="text-stone-500 italic">con dedicación.</span>
          </h1>
          
          <div className="relative max-w-lg mx-auto md:mx-0">
            {/* Comilla en beige oscuro */}
            <span className="hidden md:block absolute -top-4 -left-2 text-6xl text-[#D6D0C0] font-serif">"</span>
            <p className="text-stone-700 text-base md:text-lg leading-relaxed md:border-l-2 md:border-stone-400 md:pl-6 font-light italic">
              Soy responsable, empática y dedicada. Mi labor se basa en la <strong className="text-stone-900 font-normal">honestidad</strong> y el trabajo en equipo, impulsada siempre por el deseo de prosperar académicamente para brindar lo mejor a mis pacientes.
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 pt-4 justify-center md:justify-start">
            <a 
              href={emailLink}
              className="bg-stone-600 hover:bg-stone-700 text-white px-8 py-4 text-center font-medium transition shadow-lg rounded-sm hover:-translate-y-1 transform duration-300"
            >
              Agendar Consulta
            </a>
            <a href="#trayectoria" className="border border-stone-400 hover:bg-white hover:text-stone-800 text-stone-600 px-8 py-4 text-center font-medium transition rounded-sm">
              Ver Credenciales
            </a>
          </div>
        </div>
        ...
        <div className="flex justify-center md:justify-end">
          ...
          <div className="relative bg-white/70 backdrop-blur-md border border-white/80 p-6 md:p-12 max-w-md w-full rounded-sm shadow-xl">
            {/* Detalles decorativos en Gris */}
            <div className="absolute -top-4 -right-4 w-20 h-20 border-t-4 border-r-4 border-stone-300 rounded-tr-3xl"></div>
            <div className="absolute -bottom-4 -left-4 w-20 h-20 border-b-4 border-l-4 border-stone-300 rounded-bl-3xl"></div>
            
            <h3 className="text-stone-500 text-xs md:text-sm font-bold tracking-widest uppercase mb-2">Perfil Profesional</h3>
            <h2 className="text-2xl md:text-3xl font-serif text-stone-900 mb-6">Dra. Noelia Altamirano</h2>
            
            <ul className="space-y-4 text-stone-700 text-sm md:text-base">
              <li className="flex items-start gap-3">
                <Activity className="shrink-0 text-stone-500 mt-1" size={20} />
                <span>Médica de Planta Hospital El Cruce (Alta Complejidad)</span>
              </li>
              <li className="flex items-start gap-3">
                <Brain className="shrink-0 text-stone-500 mt-1" size={20} />
                <span>Especialista en Clínica Pediátrica (Col. Médicos / Min. Salud)</span>
              </li>
              <li className="flex items-start gap-3">
                <GraduationCap className="shrink-0 text-stone-500 mt-1" size={20} />
            ...
            <div className="mt-8 pt-8 border-t border-stone-200 flex justify-end items-center">
               <div className="flex items-center gap-2 text-stone-400 text-xs uppercase tracking-widest">
                  ...
                  <ShieldCheck className="text-stone-500" size={24} />
               </div>
            </div>
          </div>
        </div>
      ...
      <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 animate-bounce hidden md:block z-20">
        <a href="#perfil" className="text-stone-400 hover:text-stone-600 transition cursor-pointer">
...
const About = () => (
  // Fondo Beige ligeramente diferente para contraste visual
  <section id="perfil" className="py-16 md:py-24 bg-[#FDFCF8]">
    <div className="container mx-auto px-6">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white p-6 md:p-10 shadow-xl shadow-stone-200/50 border-t-4 border-stone-600 rounded-sm">
          <div className="flex flex-col md:flex-row gap-10 items-start">
             <div className="flex-1">
                <p className="text-base md:text-lg text-stone-600 leading-relaxed font-serif italic mb-6">
                  "{DATA.perfil}"
                </p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                  {['Responsabilidad', 'Empatía', 'Honestidad', 'Dedicación', 'Trabajo en Equipo', 'Superación'].map(skill => (
                    <div key={skill} className="flex items-center gap-2 text-stone-700 font-medium text-sm">
                      <div className="w-2 h-2 bg-stone-400 rounded-full"></div> {skill}
                    </div>
                  ))}
                </div>
             </div>
             <div className="md:w-1/3 w-full bg-[#F5F2EB] p-6 rounded border border-stone-200">
                <h4 className="font-bold text-stone-800 mb-4 flex items-center gap-2">
                  <Languages size={18} className="text-stone-500" /> Idiomas
                </h4>
                <ul className="space-y-3 text-sm text-stone-600 mb-6">
                  {DATA.idiomas.map((idioma, idx) => (
                    <li key={idx} className="flex justify-between border-b border-stone-200 pb-2 last:border-0 last:pb-0">
                      <span>{idioma.idioma}</span>
                      <span className="font-bold text-stone-700">{idioma.nivel}</span>
                    </li>
                  ))}
                </ul>
                <h4 className="font-bold text-stone-800 mb-2 flex items-center gap-2 text-xs uppercase tracking-wide">
                  <FileText size={14} className="text-stone-500" /> Informática
                </h4>
  ...
  <section id="servicios" className="py-16 md:py-24 bg-white">
    <div className="container mx-auto px-6">
      ...
      <div className="grid md:grid-cols-3 gap-8">
        {/* Card 1 */}
        <Card className="bg-[#F9F7F4] border-t-4 border-t-stone-400">
          <div className="w-14 h-14 bg-white rounded-full flex items-center justify-center mb-6 text-stone-600 shadow-sm border border-stone-100">
            <Activity size={28} />
          </div>
          <h3 className="text-xl font-serif text-stone-900 font-bold mb-3">Alta Complejidad & Urgencias</h3>
        ...
        {/* Card 2 - Centro destacado (Gris Oscuro) */}
        <Card className="bg-stone-800 text-white border-none transform md:-translate-y-4 shadow-2xl">
          <div className="w-14 h-14 bg-stone-700 rounded-full flex items-center justify-center mb-6 text-white">
            <Baby size={28} />
          </div>
          ...
          <ul className="text-sm space-y-2 text-stone-400">
            <li className="flex items-center gap-2"><ChevronRight size={14} className="text-stone-500"/> Control de percentilos</li>
            <li className="flex items-center gap-2"><ChevronRight size={14} className="text-stone-500"/> Desarrollo madurativo</li>
          </ul>
        </Card>
        ...
        <Card className="bg-[#F9F7F4] border-t-4 border-t-stone-400">
          ...
          <h3 className="text-xl font-serif text-stone-900 font-bold mb-3">Adolescencia & Especialidades</h3>
          <p className="text-stone-600 text-sm mb-4">
          ...
          <ul className="text-sm space-y-2 text-stone-600">
            <li className="flex items-center gap-2"><ChevronRight size={14} className="text-stone-500"/> Prevención Suicidio Adolescente</li>
            <li className="flex items-center gap-2"><ChevronRight size={14} className="text-stone-500"/> Dermatología Infantil (HNRG)</li>
          </ul>
        </Card>
      </div>
    </div>
    ...
    <div className="container mx-auto px-6 max-w-6xl">
      ...
      <div className="grid lg:grid-cols-2 gap-16">
        ...
        <div className="space-y-12">
          <div>
            <h3 className="flex items-center gap-3 text-xl font-bold text-stone-900 mb-8 pb-4 border-b border-stone-300">
              <Stethoscope className="text-stone-500" /> Experiencia Profesional
            </h3>
            <div className="space-y-8 border-l-2 border-stone-300 ml-2 md:ml-3 pl-6 md:pl-8 relative">
              {DATA.experiencia.map((item, idx) => (
                <div key={idx} className="relative">
                  <span className="absolute -left-[31px] md:-left-[39px] top-1 w-4 h-4 md:w-5 md:h-5 bg-stone-600 rounded-full border-4 border-[#F5F2EB]"></span>
                  <span className="text-xs font-bold text-stone-500 uppercase tracking-wider">{item.periodo}</span>
                  <h4 className="text-lg font-bold text-stone-800 mt-1">{item.rol}</h4>
                  <p className="text-stone-600 font-medium text-sm">{item.lugar}</p>
          ...
          <div>
            <h3 className="flex items-center gap-3 text-xl font-bold text-stone-900 mb-8 pb-4 border-b border-stone-300">
              <GraduationCap className="text-stone-500" /> Formación Académica
            </h3>
            <div className="space-y-6">
              {DATA.formacion.map((item, idx) => (
                <div key={idx} className="bg-white p-6 rounded shadow-sm border-l-4 border-stone-500">
                  ...
                  <p className="text-stone-600 font-medium text-sm">{item.institucion}</p>
                  {item.nota && <p className="text-stone-400 text-xs mt-2 font-medium italic border-t border-stone-100 pt-2">{item.nota}</p>}
                </div>
              ))}
            </div>
          </div>
        ...
        <div>
          <h3 className="flex items-center gap-3 text-xl font-bold text-stone-900 mb-8 pb-4 border-b border-stone-300">
            <BookOpen className="text-stone-500" /> Cursos y Actualizaciones
          </h3>
          
          <div className="grid gap-6">
            {Object.entries(DATA.cursosCategorizados).map(([categoria, cursos], idx) => (
              <div key={idx} className="bg-white p-6 rounded shadow-sm border border-stone-200 hover:border-stone-400 transition">
                <h4 className="font-bold text-stone-800 mb-4 text-sm uppercase tracking-wide flex items-center gap-2">
                  <Award size={16} className="text-stone-500" /> {categoria}
                </h4>
                <ul className="space-y-3">
                  {cursos.map((curso, i) => (
                    <li key={i} className="flex items-start gap-3 text-sm text-stone-600 leading-snug">
                       <span className="text-stone-400 mt-1.5 text-[8px]">●</span>
                       <span>{curso}</span>
                    </li>
                  ))}
                </ul>
...
const Footer = () => {
  ...
  return (
    // Footer gris oscuro elegante
    <footer id="contacto" className="bg-stone-900 text-white pt-16 md:pt-20 pb-10">
      <div className="container mx-auto px-6 grid md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16 border-b border-stone-800 pb-12">
        
        <div className="lg:col-span-2">
          <div className="flex items-center gap-3 mb-6">
            <div className="bg-stone-700 p-2 rounded text-white">
              <Activity size={24} />
            </div>
        ...
        <div>
          <h4 className="text-stone-500 font-bold uppercase tracking-widest text-sm mb-6">Contacto</h4>
          <ul className="space-y-4 text-stone-300">
            ...
            <li className="flex items-start gap-3">
              <MapPin size={18} className="text-stone-500 mt-1"/> 
              <span>{DATA.contacto.direccion}<br/><span className="text-xs text-stone-600">Consultorio Particular & Clínica del Niño</span></span>
            </li>
          </ul>
        </div>

        <div>
          <h4 className="text-stone-500 font-bold uppercase tracking-widest text-sm mb-6">Horarios</h4>
          ...
          <a 
            href={emailLink}
            className="w-full mt-6 bg-stone-700 hover:bg-stone-600 text-white py-3 rounded-sm font-medium transition text-sm flex justify-center items-center gap-2 shadow-lg"
          >
...
function App() {
  return (
    <div className="font-sans antialiased text-stone-800 bg-[#F5F2EB] selection:bg-stone-200 selection:text-stone-900">
      <Navbar />
      <Hero />
      <About />
      <Services />
